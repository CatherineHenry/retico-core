def get_first_instance_of_target_grounded_iu(obj, target_iu_objs:list, layers_deep=0):
    ...
    for target_iu_obj in target_iu_objs:
        if isinstance(obj, dict) and target_attr in obj:
            ...
            if isinstance(grounded_in_iu, target_iu_obj):
                # print(f"Found grounded in IU {type(grounded_in_iu)} at layer {layers_deep}")
    ...
    layers_deep+=1
    if isinstance(obj, dict) and target_attr in obj:
        return get_first_instance_of_target_grounded_iu(obj[target_attr], target_iu_objs, layers_deep=layers_deep)
    # check if a python obj and if it has the attr
    elif hasattr(obj, '__dict__') and hasattr(obj, target_attr):
        return get_first_instance_of_target_grounded_iu(getattr(obj, target_attr), target_iu_objs, layers_deep=layers_deep)
